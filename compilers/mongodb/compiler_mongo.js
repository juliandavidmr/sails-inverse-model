...
exports.generate = function(host, port, database, plurallang, folder_views, folder_models, folder_controllers) {
	return new Promise(function(resolve, reject) {
		exports.generateModels(host, port, database, plurallang).then((Models) => {
			...
			mondongo.describe(connection_url).then((described) => {
				described.forEach(elem => {
					if (elem.collection !== 'system.indexes' && elem.count > 0) {
						...
						Models.push({
							model_name: plural.pluraliza(s.camelize(elem.collection), plurallang).trim(),
