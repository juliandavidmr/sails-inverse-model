...
export async function generate(config, folder_models, folder_controllers, folder_views) {
    const schemas = await PostgresSchema.toJSON(config, config.schema)
    ...
    for (var table in schemas.tables) {
        ...
        if (schemas.tables.hasOwnProperty(table)) { //confirm data of tables
            ...
            //console.log(attrs);
            var attributes_sails: string[] = [], view_contents: string[] = [];
...
function toSailsAttribute(type_, attrib, default_value_, is_nullable_) {
    var sails_attribute_children: string[] = [];
    var content_view = {
